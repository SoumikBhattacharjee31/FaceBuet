...
export default function CreateGroupPost({open, setCurrentComponent, groupId}) {
  ...
  const handleSubmit = async (event) => {
    ...
    try{
      ...
      request_data.append("group_id", group_id)
      const response = await axios
        .post("http://localhost:8000/api/set_group_post/", request_data, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
        if ('error' in response.data){console.log(response.data.error);}
        else {setCurrentComponent('GroupPage')}
